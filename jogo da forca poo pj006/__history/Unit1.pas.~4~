...
var
   Letra: string;
begin
   Letra := InputBox('Informe uma letra', 'Letra', '');

   if Letra = '' then
      raise Exception.Create('Um letra deve ser informada.');

   JogoForca.Jogar(Letra[1], InputBox('Informe o nome do jogador', 'Nome do jogador', ''));

   Display.Caption := JogoForca.Display;

   Label1.Caption := JogoForca.GetLetrasUtilizadas;

   Memo1.Lines := JogoForca.GetEstatisticas;
...
function TForm1.GerarPalavra: string;
var
